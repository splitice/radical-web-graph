...
use Radical\Basic\DateTime\Timestamp;
...
class pChart {
	...
	/*
	 * Allow you to fix the scale, use Radical\this to bypass the automatic scaling
	 */
	 ...
	 * Compute and draw the scale
	 */
	function drawGrid($LineWidth, $Mosaic = TRUE, $R = 220, $G = 220, $B = 220, $Alpha = 100, $lineSkip = 1) {
		/*
		 * Draw mosaic
		 */
		if ($Mosaic) {
				...
				if ($SerieName == "" || $SerieName == $ColName) {
					...
					foreach ( $Data as $Key => $Values ) {
						if (isset ( $Data [$Key] [$ColName] )) {
							...
							if ($this->BuildMap) // Save point into the image map if
							                     // option activated
	...
	function drawFilledCircle($Xc, $Yc, $Height, $R, $G, $B, $Width = 0) {
		...
		for($i = 90; $i <= 270; $i = $i + $Step) {
			...
			// if (($Y1 - 1) > $yRadius)
			// imageline ( $this->Picture, $X1, $Y1 - 1, $X2 - 1, $Y2 - 1,
		// $C_Circle );
		}
		
		// Instead of drawing lines use Radical\GDs elipse to fill the center
	 ...
	 */
	function drawAntialiasPixel2($X, $Y, $R, $G, $B, $Alpha = 100, $NoFallBack = FALSE) {
		/*
		 * Process shadows
		 */
		if ($this->ShadowActive && ! $NoFallBack) {
